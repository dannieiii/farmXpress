  ...
  const products = ref([
    {
      ...
      weight: 1,
      unit: 'kg',
      image: 'https://cdn-icons-png.flaticon.com/512/1412/1412511.png',
      showOnline: true,
      onSale: false,
      preorder: false,
      preorderLimit: null,
    ...
    // Add more product examples as needed
  ]);
  
  // Filtered products based on search, category, and status
  const filteredProducts = computed(() => {
    return products.value.filter(product => {
      const matchesSearch = product.productName.toLowerCase().includes(searchQuery.value.toLowerCase());
      const matchesCategory = activeCategory.value === 'all' || product.category.toLowerCase() === activeCategory.value;
      const matchesFilter = activeFilter.value === 'All' || product.status === activeFilter.value;
